...
public class Q5 {
    ...
    static void main() {
        ...
        for(int i = 0; i < dogarray.length; i++ ){
            Dogs dog  = (Dogs) dogarray[i];

            tot = tot + dog.getWeight();
        }
        double avg = tot/dogarray.length;


        double max = dogarray[0].getWeight();
        for(int i=1;i<dogarray.length;i++){
            if (max < dogarray[i].getWeight()){
                max = dogarray[i].getWeight();
            }
        }
        ...
        for(int i=1;i<dogarray.length;i++){
            if (min > dogarray[i].getWeight()){
                min = dogarray[i].getWeight();
            }
        }
